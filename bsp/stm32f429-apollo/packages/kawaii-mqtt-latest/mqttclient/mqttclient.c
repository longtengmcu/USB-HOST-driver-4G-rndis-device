...
static int mqtt_try_do_reconnect(mqtt_client_t* c)
{
    ...
    {
        KAWAII_MQTT_LOG_I("%s:%d %s()... mqtt try connect result is -0x%04x", __FILE__, __LINE__, __FUNCTION__, -rc);
    }
...
static void mqtt_yield_thread(void *arg)
{
    ...
    while (1) {
        ...
        if (KAWAII_MQTT_CLEAN_SESSION_ERROR == rc) {
            ...
            KAWAII_MQTT_LOG_I("%s:%d %s()..., mqtt reconnect timeout....", __FILE__, __LINE__, __FUNCTION__);
        }
    }
